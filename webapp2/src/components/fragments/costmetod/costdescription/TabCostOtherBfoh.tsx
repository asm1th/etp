...
const TabCostOtherBfoh: FC = () => {
    ...
    const delItem = () => {
        ...
        dispatch(sampSlice.actions.delCostOtherBfoh(delItemKey))
    }
    ...
    const handleCostOtherBfohPropText = (key: number, name: string, value: string) => {
        dispatch(sampSlice.actions.setCostOtherBfohProp({
            key: key,
            name: name,
            value: value
        }))
    ...
    const handleCostOtherBfohPropNumber = (key: number, name: string, value: string) => {
    ...
    return (
        <>
            ...
            <Text as="div" size="l" className="mb1 weight700">
                Расчет прочих затрат до начисления накладных расходов
            </Text>
                    ...
                    <Layout className="Row mb1" key={key}>
                        <Layout flex={5} className="tar aic">
                            <TextField
                                ...
                                width="full"
                                className="mr05"
                                onChange={({ e }: any) => { handleCostOtherBfohPropText(key, e.target.name, e.target.value) }}
                        ...
                        <Layout flex={2} className="aic">
                            <TextField
                                ...
                                width="full"
                                className="RowInput textCenter"
                                onChange={({ e }: any) => { handleCostOtherBfohPropNumber(key, e.target.name, e.target.value) }}
...
export default TabCostOtherBfoh;